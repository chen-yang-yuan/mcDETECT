...
def arm_name(sample, seed=None):
    """'WT_real' or 'WT_perm_seed3' -- the arm key used across every A2b output table."""
    return f"{sample}_real" if seed is None else f"{sample}_perm_seed{seed}"


def all_arms():
    """Every arm scored by score_embedding.py, in array-index order."""
    arms = [(s, None) for s in SAMPLES]
    arms += [(s, seed) for s in SAMPLES for seed in PERM_SEEDS]
    return arms
