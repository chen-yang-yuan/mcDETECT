...
# Usage:
#   python run_Baysor.py --block 1   # multi_marker 3D all, seeds 1–10 only
#   python run_Baysor.py --block 20   # seeds 191–200
#   python run_Baysor.py              # multi_marker 3D all, all 200 seeds (or eval-only after blocks)
# Baysor is run only on multi-marker data (200 seeds); single-marker A/B/C are skipped.
PARSER = argparse.ArgumentParser(
    description="Run Baysor on multi-marker simulated data; optionally restrict to a seed block for parallel jobs."
)
PARSER.add_argument(
    "--block",
    type=int,
    default=None,
    help="Block index 1–20. Each block covers 10 seeds: 1–10, 11–20, ..., 191–200.",
)
...
LIMIT_N = None
# For full 200-seed eval (sbatch run_Baysor.sh eval), set DEBUG_SAMPLE = False.
...
# Debug: optionally run on a small subset first
DEBUG_SAMPLE = False
DEBUG_MODE = {
    "mode": "multi_marker",
    "dimension": "3D",
...
# Baysor: only multi-marker data (skip single-marker A, B, C)
inputs_df = inputs_df[
    (inputs_df["mode"] == "multi_marker") &
    (inputs_df["dimension"] == "3D") &
    (inputs_df["scenario"] == "all")
].copy().reset_index(drop=True)
print("Baysor: multi_marker 3D all only. Simulations to run:", inputs_df.shape[0])

if BLOCK is not None:
    if BLOCK < 1 or BLOCK > 20:
        raise ValueError(f"--block must be between 1 and 20, got {BLOCK}")
    block_size = 10
    start_seed = (BLOCK - 1) * block_size + 1
    end_seed = BLOCK * block_size
    inputs_df = inputs_df[inputs_df["seed"].between(start_seed, end_seed)].copy().reset_index(drop=True)
    print(f"Block filter --block {BLOCK}: seeds {start_seed}–{end_seed}, simulations to run: {inputs_df.shape[0]}")

print(inputs_df.head())

# Optional debug subset (only when not running a block job)
if DEBUG_SAMPLE and BLOCK is None:
    mask = (
...
_log_suffix = f"_block{BLOCK}" if BLOCK is not None else ""
log_path = os.path.join(BAYSOR_OUT_ROOT, f"baysor_run_log{_log_suffix}.csv")
...
index_path = os.path.join(BAYSOR_OUT_ROOT, f"baysor_spheres_index{_log_suffix}.csv")
