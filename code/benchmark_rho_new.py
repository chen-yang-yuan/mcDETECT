...
for rho in rho_values:
    ...
    #     and (3) number of unique genes per granule (acceleration: single pass, vectorized counts)
    ...
    for k in range(num_detections):
        ...
        center = row[["sphere_x", "sphere_y", "sphere_z"]].values.astype(np.float64)
        r = float(row["sphere_r"])
        distances = np.sqrt(((transcript_coords - center) ** 2).sum(axis=1))
        within_sphere = distances <= r
