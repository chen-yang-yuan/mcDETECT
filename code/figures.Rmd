...
```{r}
df <- read_parquet(here::here("output/MERSCOPE_WT_1/all_granules_local.parquet"))
...
for (scenario in c("merge", "merge_volume", "merge_jaccard", "merge_dice")) {
  ...
  # line plot
  p <- ggplot(df_plot, aes(x = .data[[thr_name]], y = num_detections)) +
    geom_line(data = df_plot, aes(x = .data[[thr_name]], y = num_detections, group = 1)) +
    geom_point(shape = 21, size = 4, color = "black", fill = "#1f77b4") +
    coord_cartesian(ylim = c(755750, NA)) +
  ...
  ggsave(here::here(paste0("output/benchmark/benchmark_rho_", scenario, "_number_of_detections.jpeg")), p, width = 9, height = 6, dpi = dpi)
...
for (scn in names(scenario_x)) {
  ...
  p_msr <- ggplot() +
    ...
    geom_point(data = df_s, aes(x = x_fct, y = mean_sphere_r), shape = 21, size = 4, color = "black", fill = "#00BA38") +
    scale_x_discrete(drop = FALSE, labels = x_labels) +
